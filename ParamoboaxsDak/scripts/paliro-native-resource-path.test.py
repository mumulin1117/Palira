...
extension PaliroLocalResources {
    static func verifyPaths() throws {
        ...
        guard case .memory(let html, _) = try resource(for: "index.html", webRoot: root),
        ...
        guard case .file = try resource(for: "plain.txt", webRoot: root) else { fatalError("Plain asset did not load") }
        ...
        for path in ["../outside.txt", "escape.txt", outside.path] {
            do { _ = try resource(for: path, webRoot: root); fatalError("Escaped root: \(path)") }
        ...
        try removeOldAssetCaches(in: base.appendingPathComponent("alias/cache"), keepingVersion: "current")
