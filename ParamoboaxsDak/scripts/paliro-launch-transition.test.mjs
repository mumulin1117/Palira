...
test('native launch art is installed before loading an opaque branded WebView', () => {
  ...
  assert.match(nativeBridge, /capacitorDidLoad\(\)[\s\S]*configureLaunchSurface\(\)[\s\S]*PaliroLaunchLocale\.current/)
...
test('the boot route and earliest HTML paint use the branded dark launch surface', () => {
  ...
  assert.match(storyboard, /red="0\.019607843137254902"[^>]+blue="0\.12941176470588237"/)
  assert.match(storyboard, /<view key="view"[\s\S]*<imageView[\s\S]*translatesAutoresizingMaskIntoConstraints="NO"/)
  assert.match(html, /var\(--paliro-launch-image,url\('\/assets\/paliro-launch-screen-ko@2x\.png'\)\)/)
})
